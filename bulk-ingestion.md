...
* There is an irrecoverable error inside of Bulk Ingestion due to an unexpected system failure
...
|--|--|
| REJECTED | This indicates a problem resulting from one of these conditions: `File is malformed`, `File is not in GZIP format`, `Uncompressed file is not in CSV format`, `A row error threshhold count was exceeded` |
