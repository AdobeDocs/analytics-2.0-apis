...
**`mvvar1` - `mvvar3`** | - No longer includes a delimiter in the schema.<br/>- All variables now use a comma (`,`) to delimit values.<br/>- Now truncates individual values to 255 bytes each, matching what is stored in a report suite.<br/>- Now truncates the total length of the field (including delimiters) to 65,535 bytes.<br/>- Now omits duplicate entries in the same hit.
...
**`tnt`** | No longer includes superfluous delimiter and value fields.
