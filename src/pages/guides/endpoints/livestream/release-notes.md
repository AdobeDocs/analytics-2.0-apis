...
description: Cumulative release notes for LiveStream data and endpoints.
---
...
Field name | Change
---|---
**`evar1` - `eVar250`** | - Counter eVars set to a string value now return a value of `1.0` instead of the string. - Now truncates values to 255 bytes, matching what is stored in a report suite.
**`events`** | - Now supports counting multiple instances of the same event. For example, if the `events` variable contains `event1,event1`, two instances are now counted instead of just counting the first instance. - Setting an event to a value of `0` omits that event entirely instead of passing `eventX=0`. - Numeric and currency events that do not have an associated value are now omitted. - Any event that is assigned a non-numeric value is now omitted. - Event serialization is now truncated to 20 characters, matching what is stored in a report suite.
...
**`mobileAudioSupport`** **`mobileVideoSupport`** | Fixed an issue that included only one supported format for devices that support multiple formats. Multiple values are separated by commas.
**`mvvar1` - `mvvar3`** | - The delimiter field now always specifies a comma (`44`) instead of the delimiter set in report suite settings. The field is now considered deprecated. - Now truncates individual values to 255 bytes each, matching what is stored in a report suite. - Now truncates the total length of the field (including delimiters) to 65,535 bytes. - Now omits duplicate entries in the same hit. -
**`products`** | - Incorrectly parsed product values are now truncated instead of erroneously creating multiple product values. - Now omits duplicate product entries in the same hit. - Product values missing a name are no longer included. - Omits events for product values if the `events` field does not also include that event. - If the same merchandising eVar appears in the same product value, the last eVar value is now used instead of the first. - Standard and conversion variable syntax merchandising eVars are now omitted from product values, as only product syntax merchandising eVars are allowed in this field.
...
**`tntAction`** | - Now excludes traffic from bots. - Now omits duplicate entries in the same hit. - Action IDs with a value of `-1` are now returned as a value of `65535`. - If an action contains multiple action IDs, only the first is used.
