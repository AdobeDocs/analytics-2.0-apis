...
`linkType` | `pe` | The type of link. Defaults to `o` if this field is empty and `linkName` contains a value. Valid values when using the `linkType` column include: **`d`**: Download link **`e`**: Exit link **`o`**: Custom link When using the `pe` query string, use: **`lnk_d`**: Download link **`lnk_e`**: Exit link **`lnk_o`**: Custom link
