...
| `type` | `string` | The virtual report suite type. System generated. **advancedVrs**: A VRS is advanced if `dataSchema` is `CacheAndMid` or `Stitched`. **vrs**: A VRS is basic if `dataSchema` is set to `Cache` |
| `dataSchema` | `string` | Defines the data schema type for the virtual report suite. **Cache**: Basic `dataSchema` type. Does not allow setting `sessionDefinition` or CDA. **CacheAndMid**: Must use this `dataSchema` if `sessionDefinition` is provided. **Stitched**: Enables [Cross-Device Analytics](https://docs.adobe.com/content/help/en/analytics/components/cda/cda-home.html). |
...
| Attribute | Type | Description |
| --- | --- | ---------- |
| `func` | `string` | Session definition option type. **inactivity**: Defines the amount of inactivity a unique visitor must have before a new visit is automatically started. **beforeEvents**: A new session starts when any of these events are fired regardless of whether a session has timed out. The new session that is created includes the event that started it. |
